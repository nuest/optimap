...
- **Persisted `Work.regions` (continents + oceans), aligned with `Work.countries`.** Global-region membership is now stored on a new many-to-many relation `Work.regions → GlobalRegion`, derived by the same **offline point-in-polygon join** pattern as countries (`works.services.regions.lookup_regions`, with `ST_MakeValid` geometry repair; **no** coastal buffer-snap since continents+oceans tile the globe). It is populated on save (a `post_save` signal `assign_work_regions`, gated by `OPTIMAP_GEOCODE_WORKS_ON_SAVE`) and by a **weekly self-healing sweep** (`works.tasks.backfill_work_regions`, scheduled automatically, staggered after the country sweep; run on demand with `python manage.py backfill_work_regions`). The region feed pages, regional subscription emails, and the `by_continent`/`by_ocean` statistics now read this M2M instead of each re-intersecting geometry with a slightly different (and previously divergent) predicate on every request. The API exposes `region_names` on each work, and the join is recorded in `Work.provenance.regions`.
- **Staff country curation on `/countries` (issue #261).** A collapsible, staff-only section on the countries page lists (paginated) the works that have a geometry but could not be matched to any country. For each work a curator can **assign a country manually** from a dropdown, or mark it **"will not be matched to a country"** (e.g. open-ocean or global studies). The exclusion is modelled with a reserved sentinel `Country` (`iso_code="ZZ"`, empty geometry, created by migration `0032_country_sentinel`) assigned via the existing `Work.countries` M2M — so the backfill's "no country" query already skips it with no schema change. The sentinel is hidden from every public country/iso enumeration (overview, `/at/`, map layer/API, statistics, `Work.country_codes`). Manual decisions are recorded in `Work.provenance.countries` (`source: "manual"`, `method: "curator_assigned"|"curator_excluded"`) plus a `country_curation` audit event, and are **preserved until the work's geometry changes**, at which point they are voided and the work re-enters automated matching. The section also has a **"Run country backfill now"** button that enqueues `works.tasks.backfill_work_countries` and shows the task id.
...
- Region landing pages (`/regions/continent/<slug>/`, `/regions/ocean/<slug>/`) no longer keep serving a stale, possibly empty page after works are linked to a region. The cached page (TTL `FEED_CACHE_HOURS`, default 24h) is now invalidated whenever `Work.regions` membership changes — both from the per-work `assign_work_regions` signal and at the end of the `backfill_work_regions` sweep — so a backfill is reflected immediately instead of after the cache expires.
