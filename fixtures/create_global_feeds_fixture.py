...
OCEANS = [
    ...
    (
        ...
        "POLYGON ((110 -30, 130 -30, 130 10, 110 10, 110 -30))",
        "South China and Easter Archipelagic Seas",
    ),
...
TWO_REGION_OVERLAPS = [
    ...
    (
        ...
        "POLYGON ((-80 25, -50 25, -50 45, -80 45, -80 25))",
        "Eastern North America and Western Atlantic",
    ),
    (
        "Africa-Indian Ocean",
        "POLYGON ((35 -20, 55 -20, 55 5, 35 5, 35 -20))",
        "East African coast and Western Indian Ocean",
    ),
    (
        "South America-Pacific",
        "POLYGON ((-85 -20, -65 -20, -65 5, -85 5, -85 -20))",
        "Western South America and Eastern Pacific",
    ),
...
MULTI_REGION_SPANS = [
    (
        "Global Ocean Survey",
        "MULTIPOLYGON (((-40 -10, -20 -10, -20 10, -40 10, -40 -10)), ((60 -20, 80 -20, 80 0, 60 0, 60 -20)), ((150 -30, 170 -30, 170 -10, 150 -10, 150 -30)))",
        "Atlantic, Indian, and Pacific Oceans",
    ),
    (
        "Trans-Atlantic Research",
        "POLYGON ((-70 20, 10 20, 10 50, -70 50, -70 20))",
        "North America, Atlantic Ocean, and Europe",
    ),
    (
        "African-Asian Monsoon Study",
        "POLYGON ((20 -10, 90 -10, 90 25, 20 25, 20 -10))",
        "Africa, Indian Ocean, and Asia",
    ),
    (
        "Pan-Pacific Study",
        "POLYGON ((110 -40, -80 -40, -80 50, 110 50, 110 -40))",
        "Asia, Pacific Ocean, North America, South America, Australia",
    ),
    (
        "Southern Hemisphere Ocean Study",
        "POLYGON ((-180 -60, 180 -60, 180 -35, -180 -35, -180 -60))",
        "Southern Ocean, Pacific, Atlantic, Indian Oceans, South America, Africa, Australia, Antarctica",
    ),
    (
        "Arctic Circumpolar Study",
        "POLYGON ((-180 65, 180 65, 180 85, -180 85, -180 65))",
        "Arctic Ocean, North America, Europe, Asia",
    ),
    (
        "Global Climate Network",
        "MULTIPOLYGON (((-120 30, -100 30, -100 45, -120 45, -120 30)), ((10 40, 30 40, 30 55, 10 55, 10 40)), ((120 -30, 140 -30, 140 -20, 120 -20, 120 -30)), ((-50 -20, -40 -20, -40 -10, -50 -10, -50 -20)))",
...
REGION_POINTS = [
    ...
    (
        "Monitoring Buoy: South China Seas",
        "POINT (120 0)",
        "ocean monitoring buoy in the South China and Easter Archipelagic Seas",
    ),
...
CROSS_REGION_LINES = [
    (
        "Migration Route: Africa to Europe",
        "LINESTRING (20 -5, 25 10, 15 35, 10 45)",
        "bird migration corridor from Africa through Mediterranean to Europe",
    ),
    (
        "Migration Route: Asia to Australia",
        "LINESTRING (100 30, 110 10, 120 -10, 130 -20)",
        "bird migration corridor from Asia to Australia",
    ),
    (
        "Shipping Lane: Atlantic Crossing",
        "LINESTRING (-75 40, -50 45, -25 50, -5 52)",
        "major shipping route across North Atlantic from North America to Europe",
    ),
    (
        "Shipping Lane: Pacific Crossing",
        "LINESTRING (140 35, 170 38, -160 40, -130 42)",
        "major shipping route across North Pacific from Asia to North America",
    ),
    (
        "Ocean Current: Gulf Stream",
        "LINESTRING (-80 25, -70 30, -50 35, -30 40, -10 50)",
        "Gulf Stream current from Gulf of Mexico to North Atlantic",
    ),
    (
        "Ocean Current: Kuroshio",
        "LINESTRING (125 25, 135 30, 145 35, 155 40)",
        "Kuroshio Current along eastern Asia into Pacific",
    ),
    (
        "Seismic Survey: Mid-Atlantic Ridge",
        "LINESTRING (-35 -30, -30 -10, -25 10, -20 30, -15 50)",
        "geological survey along Mid-Atlantic Ridge from South Atlantic to North Atlantic",
    ),
    (
        "Seismic Survey: Ring of Fire West",
        "LINESTRING (120 -10, 125 0, 130 10, 135 20, 140 30)",
        "seismic monitoring along western Pacific Ring of Fire from Indian Ocean to Pacific",
    ),
    (
        "Cable Route: Trans-Pacific",
        "LINESTRING (-120 35, -140 32, -160 30, -180 28, 170 26, 150 25)",
        "undersea telecommunications cable across Pacific from North America to Asia",
    ),
    (
        "Cable Route: Europe-Africa",
        "LINESTRING (10 55, 5 45, 0 35, -5 25, 0 10, 5 0)",
        "undersea cable from Europe through Atlantic to Africa",
    ),
]
...
COMPLEX_POLYGONS = [
    (
        "Triangular Survey Area: Mediterranean",
        "POLYGON ((10 35, 20 45, 5 42, 10 35))",
        "triangular research zone in Mediterranean Region",
    ),
    (
        "Pentagon Study Region: Central Europe",
        "POLYGON ((10 48, 15 50, 17 47, 12 44, 8 46, 10 48))",
        "five-sided ecological study area in Central Europe",
    ),
    (
        "Concave Polygon Zone: Southeast Asia",
        "POLYGON ((100 5, 105 5, 105 10, 103 8, 101 10, 100 10, 100 5))",
        "irregularly shaped coastal research area",
    ),
    (
        "Protected Area with Exclusion Zone: Amazon",
        "POLYGON ((-65 -5, -60 -5, -60 0, -65 0, -65 -5), (-63 -3, -62 -3, -62 -2, -63 -2, -63 -3))",
        "conservation area with restricted inner zone in Amazon rainforest",
    ),
    (
        "Star-shaped Survey: Arabian Peninsula",
        "POLYGON ((50 22, 51 23, 52 22, 51 21, 52 20, 51 19, 50 20, 49 19, 48 20, 49 21, 48 22, 49 23, 50 22))",
        "multi-pronged geological survey region",
    ),
]
...
MIXED_GEOMETRIES = [
    # Point only (single)
    (
        "Point-only Study: Remote Island",
        "GEOMETRYCOLLECTION (POINT (-10 75))",
        "single monitoring station on remote Arctic island",
    ),
    # Line only (single)
    (
        "Line-only Survey: Shipping Route",
        "GEOMETRYCOLLECTION (LINESTRING (-15 70, 5 72, 25 75))",
        "linear shipping route survey in North Atlantic",
    ),
    # Polygon only (single)
    (
        "Polygon-only Region: Coastal Zone",
        "GEOMETRYCOLLECTION (POLYGON ((0 80, 10 80, 10 85, 0 85, 0 80)))",
        "coastal research zone in Arctic Ocean",
    ),
    # Point + Line
    (
        "Point-Line Study: River Monitoring",
        "GEOMETRYCOLLECTION (POINT (0 5), LINESTRING (-5 0, 0 5, 5 10))",
        "river monitoring with station and flow path",
    ),
    # Point + Polygon
    (
        "Point-Polygon Study: Harbor Analysis",
        "GEOMETRYCOLLECTION (POINT (100 10), POLYGON ((98 8, 102 8, 102 12, 98 12, 98 8)))",
        "harbor with central buoy and boundary zone",
    ),
    # Line + Polygon
    (
        "Line-Polygon Study: Coastal Transect",
        "GEOMETRYCOLLECTION (LINESTRING (80 27, 85 29, 90 28), POLYGON ((82 26, 88 26, 88 30, 82 30, 82 26)))",
        "coastal transect through study area",
    ),
    # Point + Line + Polygon (full combination)
    (
        "Multi-site Arctic Study",
        "GEOMETRYCOLLECTION (POINT (-10 75), LINESTRING (-15 70, 5 72, 25 75), POLYGON ((0 80, 10 80, 10 85, 0 85, 0 80)))",
        "integrated Arctic research with monitoring stations, survey transects, and study areas",
    ),
    # Multiple Points + Line
    (
        "Multi-Point-Line: Island Network",
        "GEOMETRYCOLLECTION (POINT (160 -5), POINT (165 0), POINT (170 5), LINESTRING (158 -8, 172 8))",
        "island monitoring network with connection route",
    ),
    # Multiple Points + Polygon
    (
        "Multi-Point-Polygon: Lake Study",
        "GEOMETRYCOLLECTION (POINT (-75 20), POINT (-70 18), POLYGON ((-80 15, -60 15, -60 25, -80 25, -80 15)))",
        "lake study with sampling stations and boundary",
    ),
    # Multiple Lines + Polygon
    (
        "Multi-Line-Polygon: Watershed Analysis",
        "GEOMETRYCOLLECTION (LINESTRING (50 20, 52 22), LINESTRING (51 19, 52 21), POLYGON ((48 18, 54 18, 54 24, 48 24, 48 18)))",
        "watershed with multiple streams and catchment area",
    ),
]
...
EXTREME_SCALE_GEOMETRIES = [
    (
        "Micro-site Study: Urban Park",
        "POLYGON ((13.40500 52.52000, 13.40510 52.52000, 13.40510 52.52005, 13.40500 52.52005, 13.40500 52.52000))",
        "very small urban ecology study (sub-meter precision)",
    ),
    (
        "Continental-scale Transect",
        "LINESTRING (-120 25, -80 30, -40 35, 0 40, 40 45, 80 50, 120 55)",
        "global east-west transect spanning multiple continents",
    ),
]
...
MULTI_GEOMETRY_TYPES = [
    (
        "Scattered Monitoring Network: Pacific Islands",
        "MULTIPOINT ((160 -10), (165 -5), (170 0), (175 5), (180 10))",
        "distributed ocean monitoring stations across Pacific",
    ),
    (
        "Multi-route Shipping Analysis",
        "MULTILINESTRING ((140 30, 150 32, 160 33), (142 28, 152 29, 162 30), (138 32, 148 34, 158 35))",
        "parallel shipping corridor analysis in Northwest Pacific",
    ),
    (
        "Fragmented Habitat Study: Indonesia",
        "MULTIPOLYGON (((120 -5, 122 -5, 122 -3, 120 -3, 120 -5)), ((124 -4, 126 -4, 126 -2, 124 -2, 124 -4)), ((128 -6, 130 -6, 130 -4, 128 -4, 128 -6)))",
        "island biogeography across separated land masses",
    ),
]
...
def create_publication(
    ...
    if has_openalex:
        if random.random() > 0.2:  # 80% have full OpenAlex match
            ...
            openalex_ids = json.dumps(
                {"doi": f"https://doi.org/{doi}", "pmid": f"{38000000 + pk}" if random.random() > 0.5 else None}
            )
...
def main():
    ...
    for i, (region_name, geometry, description) in enumerate(CONTINENTS):
        ...
        print(
            f"  [{pk}] {region_name}: {len(pub['fields']['authors'])} authors, {len(pub['fields']['keywords'])} keywords, {len(pub['fields']['topics'])} topics"
        )
    ...
    for i, (title_suffix, geometry, description) in enumerate(MULTI_REGION_SPANS):
        ...
        fixture_data.append(pub)
        print(
            f"  [{pk}] {title_suffix}: {len(pub['fields']['authors'])} authors, {len(pub['fields']['keywords'])} keywords, {len(pub['fields']['topics'])} topics"
        )
    ...
    for i, (title, geometry, description) in enumerate(REGION_POINTS):
        ...
        fixture_data.append(pub)
        print(
            f"  [{pk}] {title}: {len(pub['fields']['authors'])} authors, {len(pub['fields']['keywords'])} keywords, {len(pub['fields']['topics'])} topics"
        )
