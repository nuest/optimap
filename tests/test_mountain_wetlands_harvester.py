...
class MountainWetlandsHarvesterTests(TestCase):
    ...
    def setUp(self):
        self.collection = Collection.objects.create(
            identifier='mountain-wetlands', name='Mountain Wetlands', is_published=True,
        )
    ...
    def test_works_are_added_to_source_collection(self):
        ...
        for w in Work.objects.all():
            self.assertIn(self.collection, list(w.collections.all()),
                          f'work {w.pk} should be in mountain-wetlands collection')
...
class MountainWetlandsOpenAlexMatchTests(TestCase):
    ...
    def test_candidate_match_with_api_doi_keeps_api_doi(self):
        candidate_fields = {
            'openalex_id': None,
            'openalex_match_info': [
                {'openalex_id': 'https://openalex.org/W999', 'title': 'Similar but not exact', 'match_type': 'title+author'},
            ],
        }
        with self._patched_session(self.payload), \
             patch('works.harvesting.mountain_wetlands.build_openalex_fields',
