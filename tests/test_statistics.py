...
from works.models import Source, Work, StatisticsSnapshot, SourceCoverageSnapshot
...
class StatisticsAPITests(TestCase):
    ...
    def test_contains_required_keys(self):
        resp = self.client.get(STATS_URL)
        data = resp.json()
        for key in ('total_works', 'published_works', 'sources', 'collections',
                    'works_by_status', 'by_continent', 'by_ocean',
                    'by_country', 'by_publisher', 'by_journal',
                    'total_works_for_user', 'computed_at', 'next_update'):
