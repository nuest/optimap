...
class CollectionsIndexCountsTests(TestCase):
    ...
    def test_admin_sees_per_status_breakdown(self):
        admin = User.objects.create_user(
            username="admin-counts@example.com",
...
class CollectionDetailPageTests(TestCase):
    def setUp(self):
        ...
        self.source = Source.objects.create(
            name="MW Source",
            url_field="https://example.com/api",
    ...
    def test_curator_response_is_not_cached_by_middleware(self):
        ...
        admin = User.objects.create_user(
            username="admin-cache@example.com",
...
class PublishCollectionWorksTests(TestCase):
    ...
    def test_button_hidden_when_no_publishable_works(self):
        ...
        self.harvested.status = "p"
        self.harvested.save()
        self.contributed.status = "p"
        self.contributed.save()
        self.client.force_login(self.admin)
        resp = self.client.get(reverse("optimap:collection-page", args=[self.col.identifier]))
...
class ContributeCollectionFilterTests(TestCase):
    def setUp(self):
        ...
        self.other_needs = Work.objects.create(
            title="Other needs geom",
            status="h",
            doi="10.1234/other-need",
        )
        self.other_needs.collections.add(self.other)
        self.loose_needs = Work.objects.create(
            title="Loose needs geom",
            status="h",
            doi="10.1234/loose-need",
        )
...
class ProvenanceHelperTests(TestCase):
    def test_append_event_creates_events_array(self):
        work = Work.objects.create(title="X", status="d", doi="10.1234/x", geometry=GeometryCollection())
        append_event(work, "publish", user_id=1, user_email="admin@x.com", status_from="c", status_to="p")
        work.save()
...
class CollectionCuratorManagementTests(TestCase):
    ...
    def setUp(self):
        ...
        self.remove_url = reverse("optimap:collection-remove-curator", args=[self.col.id, self.curator.id])
    ...
    def test_remove_non_curator_returns_400(self):
        self.client.force_login(self.admin)
        url = reverse("optimap:collection-remove-curator", args=[self.col.id, self.other_user.id])
        resp = self.client.post(url)
        self.assertEqual(resp.status_code, 400)
...
class SourceCollectionPropagationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.xml_bytes = (Path(__file__).resolve().parent / "harvesting" / "source_1" / "oai_dc.xml").read_bytes()
    ...
    def test_source_collection_propagates_to_harvested_works(self):
        ...
        for w in works:
            self.assertIn(
                col, list(w.collections.all()), f"work {w.pk} should have been added to the source collection"
            )
...
class SourceSourceTypeAndScheduleTests(TestCase):
    ...
    def test_zero_interval_does_not_create_schedule(self):
        ...
        s = Source.objects.create(name="S3", url_field="https://example.com/oai", source_type="oai-pmh")
    ...
    def test_save_dispatches_to_correct_task_for_rss(self):
        ...
        s = Source.objects.create(
            name="S4",
            url_field="https://example.com/feed.rss",
            source_type="rss",
...
class ProvenanceEndpointTests(TestCase):
    ...
    FULL_PROVENANCE = {
        ...
        "events": [
            ...
            {"type": "contribution", "at": "2026-01-02T10:00:00+00:00", "user_id": 42, "kind": "spatial"},
    ...
    def setUp(self):
        ...
        self.other_curator = User.objects.create_user(
            username="othercurator@example.com",
            email="othercurator@example.com",
            password="pw",
        )
