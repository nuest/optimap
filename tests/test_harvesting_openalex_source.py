...
class ResolveOpenAlexSourceIdTests(TestCase):
    ...
    def test_prefers_openalex_id_field(self):
        s = self._src(openalex_id="S4210203054", url_field="https://example.test/no-s-here")
        self.assertEqual(_resolve_openalex_source_id(s), "S4210203054")

    def test_accepts_full_url_in_openalex_id(self):
        s = self._src(openalex_id="https://openalex.org/S4210203054", url_field="https://example.test/no-s-here")
        self.assertEqual(_resolve_openalex_source_id(s), "S4210203054")
...
class OpenAlexItemToKwargsTests(TestCase):
    ...
    def test_invalid_publication_date_becomes_none(self):
        out = _openalex_item_to_work_kwargs(
            self._item(publication_date="not-a-date"),
            self.source,
            self.event,
        )
