...
class RSS20SpecificationTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.pub_many_authors = Work.objects.create(
            ...
            authors=[f"Author{i}" for i in range(1, 15)],  # 14 authors - should trigger "et al."
        )
    ...
    def _fetch_rss_feed(self):
        ...
        self.assertEqual(
            response["Content-Type"], "application/rss+xml; charset=utf-8", "RSS feed should have correct Content-Type"
        )
    ...
    def test_rss_root_element(self):
        ...
        self.assertEqual(root.get("version"), "2.0", "RSS version should be 2.0")
    ...
    def test_rss_namespaces(self):
        ...
        self.assertIn('xmlns:atom="http://www.w3.org/2005/Atom"', xml_content, "RSS should declare Atom namespace")
        self.assertIn(
            'xmlns:georss="http://www.georss.org/georss"', xml_content, "RSS should declare GeoRSS namespace"
        )
        self.assertIn('xmlns:gml="http://www.opengis.net/gml"', xml_content, "RSS should declare GML namespace")
    ...
    def test_rss_channel_atom_link(self):
        ...
        self.assertTrue(self_link.get("href").endswith(".rss"), "atom:link href should end with .rss")
        # At least one self link should have the proper type
        typed_self_links = [
            link for link in atom_links if link.get("rel") == "self" and link.get("type") == "application/rss+xml"
        ]
        self.assertGreater(len(typed_self_links), 0, "At least one atom:link should have type='application/rss+xml'")
    ...
    def test_rss_item_required_elements(self):
        ...
        for i, item in enumerate(items):
            ...
            self.assertTrue(has_title or has_description, f"Item {i} should have either <title> or <description>")
    ...
    def test_rss_item_link(self):
        ...
        for item in items:
            ...
            self.assertTrue(link.text.startswith("http"), f"Item link should be a valid URL: {link.text}")
    ...
    def test_rss_item_guid(self):
        ...
        for item in items:
            ...
            if guid is not None:
                self.assertTrue(len(guid.text) > 0, "GUID should not be empty if present")
    ...
    def test_rss_item_pubdate_format(self):
        ...
        for item in items:
            ...
            if pubdate is not None and pubdate.text:
                ...
                self.assertGreaterEqual(len(date_parts), 5, f"pubDate should be in RFC 822 format: {pubdate.text}")
    ...
    def test_rss_item_author_with_openalex(self):
        ...
        for item in items:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(creator, "Item with authors should have dc:creator")
    ...
    def test_rss_item_author_et_al_threshold(self):
        ...
        for item in items:
            ...
            if title.text == "Publication With Many Authors":
                ...
                self.assertIn("et al.", creator.text, "Items with >10 authors should use 'et al.'")
                break
        else:
    ...
    def test_rss_item_no_author(self):
        ...
        for item in items:
            ...
            if title.text == "Publication Without Authors":
                creator = item.find("dc:creator", ns)
                self.assertIsNone(creator, "Item without authors should not have dc:creator")
                break
        else:
    ...
    def test_rss_item_source_element(self):
        ...
        for item in items:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(source, "Item with source should have <source> element")

                source_title = source.find("title")
                self.assertIsNotNone(source_title, "Source should have <title>")
                self.assertEqual(source_title.text, "Test Journal", "Source title should match publication source")

                source_url = source.find("url")
                self.assertIsNotNone(source_url, "Source should have <url>")
                break
        else:
            self.fail("Could not find test item 'Complete Publication Test'")
    ...
    def test_rss_item_categories(self):
        ...
        for item in items:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertGreater(len(categories), 0, "Item with keywords/topics should have categories")
    ...
    def test_rss_georss_point(self):
        ...
        for item in items:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(georss_point, "Item with point geometry should have georss:point")

                # Point should be in "lat lon" format
                coords = georss_point.text.split()
                self.assertEqual(len(coords), 2, "georss:point should have 2 coordinates (lat lon)")

                # Verify coordinates are numeric
                lat, lon = coords
                self.assertTrue(-90 <= float(lat) <= 90, "Latitude should be in valid range")
                self.assertTrue(-180 <= float(lon) <= 180, "Longitude should be in valid range")
                break
        else:
            self.fail("Could not find test item 'Complete Publication Test'")
    ...
    def test_rss_georss_polygon(self):
        ...
        for item in items:
            ...
            if title.text == "Publication With Many Authors":
                ...
                self.assertIsNotNone(georss_polygon, "Item with polygon geometry should have georss:polygon")

                # Polygon should be space-separated lat/lon pairs
                coords = georss_polygon.text.split()
                self.assertGreater(len(coords), 0, "georss:polygon should have coordinates")
                self.assertEqual(
                    len(coords) % 2, 0, "georss:polygon should have even number of values (lat/lon pairs)"
                )
                break
        else:
            self.fail("Could not find test item 'Publication With Many Authors'")
    ...
    def test_rss_georss_line(self):
        ...
        for item in items:
            ...
            if title.text == "Publication With LineString":
                ...
                self.assertIsNotNone(georss_line, "Item with line geometry should have georss:line")

                # Line should be space-separated lat/lon pairs
                coords = georss_line.text.split()
                self.assertGreaterEqual(len(coords), 4, "georss:line should have at least 2 points (4 values)")
                self.assertEqual(len(coords) % 2, 0, "georss:line should have even number of values (lat/lon pairs)")
                break
        else:
...
class Atom10SpecificationTestCase(TestCase):
    ...
    def test_atom_root_element(self):
        ...
        # Check namespace
        self.assertIn("http://www.w3.org/2005/Atom", root.tag, "Feed should use Atom namespace")

    def test_atom_namespaces(self):
        """Test Atom feed declares required namespaces."""
        xml_content = self._fetch_atom_feed()

        self.assertIn("http://www.w3.org/2005/Atom", xml_content, "Atom feed should have Atom namespace")
        self.assertIn(
            'xmlns:georss="http://www.georss.org/georss"', xml_content, "Atom feed should declare GeoRSS namespace"
        )
    ...
    def test_atom_feed_link_self(self):
        ...
        self.assertTrue(self_link.get("href").endswith(".atom"), "Self link should point to .atom file")
    ...
    def test_atom_entry_required_elements(self):
        ...
        for i, entry in enumerate(entries):
            ...
            self.assertTrue(len(entry_id.text) > 0, f"Entry {i} id should not be empty")

            title = entry.find("atom:title", ns)
            self.assertIsNotNone(title, f"Entry {i} should have <title>")
            self.assertTrue(len(title.text) > 0, f"Entry {i} title should not be empty")

            updated = entry.find("atom:updated", ns)
            self.assertIsNotNone(updated, f"Entry {i} should have <updated>")
            self.assertTrue(len(updated.text) > 0, f"Entry {i} updated should not be empty")
    ...
    def test_atom_entry_author(self):
        ...
        for entry in entries:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(author_name, "Author should have <name>")
                self.assertTrue(len(author_name.text) > 0, "Author name should not be empty")
                break
        else:
    ...
    def test_atom_entry_link(self):
        ...
        for entry in entries:
            ...
            self.assertGreater(len(links), 0, "Entry should have at least one link")
            ...
            for link in links:
                if link.get("rel") == "alternate" or not link.get("rel"):
                    ...
                    self.assertTrue(href.startswith("http"), "Link href should be a valid URL")
                    break
    ...
    def test_atom_entry_summary(self):
        ...
        for entry in entries:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(summary, "Entry with abstract should have <summary>")
                self.assertTrue(len(summary.text) > 0, "Summary should not be empty")
                break
    ...
    def test_atom_entry_source(self):
        ...
        for entry in entries:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(source, "Entry with source should have <source> element")
    ...
    def test_atom_entry_category(self):
        ...
        for entry in entries:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertGreater(len(categories), 0, "Entry with keywords should have categories")

                # Categories should have term attribute
                for category in categories:
                    term = category.get("term")
                    self.assertIsNotNone(term, "Category should have term attribute")
                break
        else:
            self.fail("Could not find test entry 'Complete Publication Test'")
    ...
    def test_atom_georss_point(self):
        ...
        for entry in entries:
            ...
            if title.text == "Complete Publication Test":
                ...
                self.assertIsNotNone(georss_point, "Entry with point geometry should have georss:point")

                # Verify format
                coords = georss_point.text.split()
                self.assertEqual(len(coords), 2, "georss:point should have 2 coordinates")
                break
        else:
            self.fail("Could not find test entry 'Complete Publication Test'")
