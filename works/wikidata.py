...
def build_statements(work):
    ...
    if work.timeperiod_startdate and check_property_exists(P_PERIOD_START):
        add_time_claims(work.timeperiod_startdate, get_local_property_id(P_PERIOD_START), statements, is_end_date=False)
    ...
    if work.source:
        ...
        if check_property_exists(P_JOURNAL_NAME):
            statements.append(MonolingualText(prop_nr=get_local_property_id(P_JOURNAL_NAME), text=work.source.name, language="en"))
...
def upsert_publication(work, wikibase_integrator, dryrun=False):
    ...
    try:
        ...
        if not existing_qid and work.openalex_id:
            ...
            if existing_qid:
                logger.info(f"Found existing item {existing_qid} by OpenAlex ID {work.openalex_id} (DOI lookup failed or no DOI)")
        ...
        if existing_qid:
            ...
            try:
                ...
                if new_statements:
                    ...
                    logger.info(f"Updated Wikidata item {existing_qid} for work {work.id} - added {len(added_fields)} properties")
                else:
                    summary = f"No new properties to add (all {len(exported_fields)} fields already exist)"
                    logger.info(f"Wikidata item {existing_qid} for work {work.id} already has all properties - no update needed")
...
def _export_works_to_wikidata_internal(works, progress_callback=None, dryrun=False):
    ...
    for idx, work in enumerate(works_list, 1):
        ...
        if action == "created":
            ...
            stats['errors'] += 1
            logger.warning(f"{mode_label}Error exporting work {work.id}: {log_entry.error_message[:100] if log_entry and log_entry.error_message else 'Unknown error'}")
