...
from wikibaseintegrator.datatypes import ExternalID, GlobeCoordinate, Item, MonolingualText, String, Time
...
P_PUBLICATION_DATE = "P577"  # work date
    ...
    try:
        ...
        for i in range(0, len(property_ids), batch_size):
            ...
            entities_response = requests.get(
                settings.WIKIBASE_API_URL,
                params={"action": "wbgetentities", "ids": "|".join(batch), "format": "json"},
                headers={"User-Agent": settings.WIKIBASE_USER_AGENT},
                timeout=30,
            )
        ...
        logger.info(
            f"Built property ID mapping with {len(_property_id_mapping)} properties via equivalent property claims"
        )
        ...
        standard_properties = [
            ...
            P_JOURNAL,
            P_GEOMETRY,
            P_NORTHERNMOST_POINT,
            P_SOUTHERNMOST_POINT,
            P_EASTERNMOST_POINT,
            P_WESTERNMOST_POINT,
            P_INSTANCE_OF,
            P_KEYWORDS,
        ...
        for wikidata_prop_id in standard_properties:
            ...
            for prop_id in property_ids:
                ...
                if local_label == wikidata_label:
                    _property_id_mapping[wikidata_prop_id] = prop_id
                    logger.debug(
                        f"Mapped property via name matching: {wikidata_prop_id} ({wikidata_label}) -> {prop_id}"
                    )
                    ...
                    try:
                        ...
                        if not has_equivalent_claim:
                            # Get CSRF token
                            token_params = {"action": "query", "meta": "tokens", "type": "csrf", "format": "json"}
                            ...
                            if claim_data.get("success") == 1:
                                logger.info(
                                    f"Added equivalent property claim to {prop_id} linking to {wikidata_property_url}"
                                )
                            else:
                                logger.warning(f"Could not add equivalent property claim to {prop_id}: {claim_data}")
                    except Exception as e:
                        logger.warning(
                            f"Error adding equivalent property claim to name-matched property {prop_id}: {e}"
                        )

                    break

        logger.info(
            f"Built complete property ID mapping with {len(_property_id_mapping)} properties (equivalent claims + name matching)"
        )
        return _property_id_mapping
...
def fetch_property_metadata_from_wikidata(property_id):
    ...
    try:
        ...
        params = {"action": "wbgetentities", "ids": property_id, "format": "json", "languages": "en"}
        ...
        metadata = {"label": label, "description": description, "datatype": datatype}
...
def create_property_in_wikibase(property_id):
    ...
    try:
        ...
        if "search" in search_data:
            for result in search_data["search"]:
                if result.get("label", "").lower() == meta["label"].lower():
                    ...
                    logger.info(
                        f"Property with label '{meta['label']}' already exists as {existing_id} - will use existing property and add equivalent claim if needed"
                    )
                    ...
                    try:
                        ...
                        if equivalent_property_id:
                            ...
                            get_params = {"action": "wbgetentities", "ids": existing_id, "format": "json"}
                            ...
                            if not has_claim:
                                ...
                                if claim_data.get("success") == 1:
                                    logger.info(
                                        f"Added equivalent property claim to existing property {existing_id} linking to {wikidata_property_url}"
                                    )
                                else:
                                    logger.warning(
                                        f"Could not add equivalent property claim to {existing_id}: {claim_data}"
                                    )
                    except Exception as e:
                        logger.warning(
                            f"Error adding equivalent property claim to existing property {existing_id}: {e}"
                        )
        ...
        property_data = {
            "labels": {"en": {"language": "en", "value": meta["label"]}},
            "descriptions": {"en": {"language": "en", "value": meta["description"]}},
            "datatype": meta["datatype"],
        }

        logger.debug(
            f"Creating property {property_id} with label '{meta['label']}' and datatype '{meta['datatype']}' (shallow copy, no claims)"
        )
...
def check_property_exists(wikidata_property_id):
    ...
    try:
        ...
        response = requests.get(
            settings.WIKIBASE_API_URL,
            params={"action": "wbgetentities", "ids": wikidata_property_id, "format": "json"},
            headers={"User-Agent": settings.WIKIBASE_USER_AGENT},
            timeout=10,
        )
        data = response.json()
        ...
        exists = wikidata_property_id in data.get("entities", {}) and "missing" not in data.get("entities", {}).get(
...
def check_item_exists(item_id):
    ...
    try:
        response = requests.get(
            settings.WIKIBASE_API_URL,
            params={"action": "wbgetentities", "ids": item_id, "format": "json"},
            headers={"User-Agent": settings.WIKIBASE_USER_AGENT},
            timeout=10,
        )
        data = response.json()
...
def build_statements(work):
    """
    Build comprehensive list of Wikidata statements from work data.
    ...
    if work.title:
        if not check_property_exists(P_TITLE):
            ...
            statements.append(MonolingualText(prop_nr=get_local_property_id(P_TITLE), text=work.title, language="en"))
            exported_fields.append("title")

    # Work date (required)
    if work.publicationDate:
        if not check_property_exists(P_PUBLICATION_DATE):
            missing_properties.append(f"{P_PUBLICATION_DATE} (work date)")
        else:
            iso_date = work.publicationDate.isoformat()
    ...
    # Abstract
    if work.abstract and check_property_exists(P_ABSTRACT):
        # Truncate if too long (Wikidata has limits)
        abstract_text = work.abstract[:5000] if len(work.abstract) > 5000 else work.abstract
        statements.append(String(prop_nr=get_local_property_id(P_ABSTRACT), value=abstract_text))
        exported_fields.append("abstract")

    # DOI
    if work.doi and check_property_exists(P_DOI):
        statements.append(ExternalID(prop_nr=get_local_property_id(P_DOI), value=work.doi))
        exported_fields.append("doi")

    # URL
    if work.url and check_property_exists(P_URL):
        statements.append(Url(prop_nr=get_local_property_id(P_URL), value=work.url))
    ...
    if check_property_exists(P_AUTHOR_STRING):
        ...
        elif work.created_by:
            statements.append(String(prop_nr=get_local_property_id(P_AUTHOR_STRING), value=work.created_by.username))
        ...
        if work.topics:
            for topic in work.topics:
                if topic and topic.strip():
                    statements.append(
                        String(prop_nr=get_local_property_id(P_KEYWORDS), value=f"Topic: {topic.strip()}")
                    )
            exported_fields.append("topics")

    # Time period - start date
    if work.timeperiod_startdate and check_property_exists(P_PERIOD_START):
        add_time_claims(
            work.timeperiod_startdate, get_local_property_id(P_PERIOD_START), statements, is_end_date=False
        )
        exported_fields.append("timeperiod_start")

    # Time period - end date
    if work.timeperiod_enddate and check_property_exists(P_PERIOD_END):
        add_time_claims(work.timeperiod_enddate, get_local_property_id(P_PERIOD_END), statements, is_end_date=True)
    ...
    if work.source:
        ...
        if check_property_exists(P_JOURNAL_NAME):
            statements.append(
                MonolingualText(prop_nr=get_local_property_id(P_JOURNAL_NAME), text=work.source.name, language="en")
            )
            exported_fields.append("source_name")

        # If source has ISSN-L
        if work.source.issn_l and check_property_exists(P_ISSN_L):
            statements.append(ExternalID(prop_nr=get_local_property_id(P_ISSN_L), value=work.source.issn_l))
            exported_fields.append("source_issn_l")

    # OpenAlex ID
    if work.openalex_id and check_property_exists(P_OPENALEX_ID):
        # Clean the ID (remove URL prefix if present)
        openalex_clean = work.openalex_id.replace("https://openalex.org/", "")
    ...
    if work.openalex_ids and isinstance(work.openalex_ids, dict):
        if work.openalex_ids.get("pmid") and check_property_exists(P_PMID):
            pmid = str(work.openalex_ids["pmid"]).replace("https://pubmed.ncbi.nlm.nih.gov/", "")
            statements.append(ExternalID(prop_nr=get_local_property_id(P_PMID), value=pmid))
            exported_fields.append("pmid")

        if work.openalex_ids.get("pmcid") and check_property_exists(P_PMC):
            pmcid = str(work.openalex_ids["pmcid"]).replace("https://www.ncbi.nlm.nih.gov/pmc/articles/", "")
    ...
    if work.openalex_is_retracted and check_property_exists(P_RETRACTED):
        ...
        try:
            ...
            if check_property_exists(P_GEOMETRY):
                ...
                if center:
                    ...
                    logger.debug(f"Added center coordinate for work {work.id}: ({lon}, {lat})")
                else:
                    logger.warning(f"Could not calculate center coordinate for work {work.id}")

            # Add extreme points (northernmost, southernmost, easternmost, westernmost)
            extreme_points = work.get_extreme_points()
            if extreme_points:
                # Northernmost point
                if extreme_points["north"] and check_property_exists(P_NORTHERNMOST_POINT):
                    ...
                    logger.debug(f"Added northernmost point for work {work.id}: ({lon}, {lat})")
                ...
                if extreme_points["south"] and check_property_exists(P_SOUTHERNMOST_POINT):
                    ...
                    logger.debug(f"Added southernmost point for work {work.id}: ({lon}, {lat})")
                ...
                if extreme_points["east"] and check_property_exists(P_EASTERNMOST_POINT):
                    ...
                    logger.debug(f"Added easternmost point for work {work.id}: ({lon}, {lat})")
                ...
                if extreme_points["west"] and check_property_exists(P_WESTERNMOST_POINT):
                    ...
                    logger.debug(f"Added westernmost point for work {work.id}: ({lon}, {lat})")

        except Exception as e:
            logger.warning(f"Error processing geometry for work {work.id}: {e}")

    # Log how many fields were checked vs exported
    logger.info(f"Built {len(statements)} statements from {len(exported_fields)} fields for work {work.id}")

    return statements, exported_fields


def create_export_log(work, action, qid=None, exported_fields=None, error_message=None, summary=None, endpoint=None):
    """
...
def upsert_publication(work, wikibase_integrator, dryrun=False):
    """
    Create or update a single Work on Wikibase with comprehensive logging.

    Args:
        work: Work object to export
    ...
    try:
        # Build statements
        statements, exported_fields = build_statements(work)

        # Check for existing item by DOI first, then fall back to OpenAlex ID
        existing_qid = None
        if work.doi:
            existing_qid = find_local_item_by_doi(work.doi)
            if existing_qid:
                logger.debug(f"Found existing item {existing_qid} by DOI {work.doi}")

        # Fallback to OpenAlex ID if DOI didn't find a match
        if not existing_qid and work.openalex_id:
            existing_qid = find_local_item_by_openalex_id(work.openalex_id)
            if existing_qid:
                logger.info(
                    f"Found existing item {existing_qid} by OpenAlex ID {work.openalex_id} (DOI lookup failed or no DOI)"
                )
        ...
        if dryrun:
            ...
            if existing_qid:
                ...
                logger.info(f"[DRY-RUN] Would update Wikidata item {existing_qid} for work {work.id}")
            else:
                action = "created"
                summary = f"[DRY-RUN] Would create with {len(exported_fields)} fields: {', '.join(exported_fields)}"
                logger.info(f"[DRY-RUN] Would create new Wikidata item for work {work.id}")
        ...
        if existing_qid:
            ...
            try:
                ...
                for i, statement in enumerate(statements):
                    ...
                    if prop_id not in existing_properties:
                        ...
                        logger.debug(
                            f"Skipping property {prop_id} ({field_name}) - already exists on item {existing_qid}"
                        )
                ...
                if new_statements:
                    ...
                    try:
                        ...
                        if isinstance(json_data, dict):
                            ...
                            logger.warning("get_json() didn't return a dict, using fallback write method")
                            entity.write(
                                summary=f"Add {len(new_statements)} missing properties via OptimapBot", clear=False
                            )
                    ...
                    logger.info(
                        f"Updated Wikidata item {existing_qid} for work {work.id} - added {len(added_fields)} properties"
                    )
                else:
                    summary = f"No new properties to add (all {len(exported_fields)} fields already exist)"
                    logger.info(
                        f"Wikidata item {existing_qid} for work {work.id} already has all properties - no update needed"
                    )
            ...
            try:
                ...
                entity.labels.set("en", work.title[:250])  # Wikidata label limit
                entity.descriptions.set("en", "Work imported from OPTIMAP")
                entity.claims.add(statements)

                entity_result = entity.write(summary="Create work via OptimapBot - comprehensive metadata")
                ...
                log_entry = create_export_log(
                    work=work, action="created", qid=created_qid, exported_fields=exported_fields, summary=summary
                )

                logger.info(f"Created Wikidata item {created_qid} for work {work.id}")
        ...
        log_entry = create_export_log(
            work=work, action="error", error_message=detailed_error, summary=f"Export failed: {error_type}"
        )
        logger.error(f"Error exporting work {work.id} to Wikidata: {short_error}")
        logger.debug(f"Full traceback for work {work.id}:\n{error_traceback}")
        return "error", None, log_entry


def _export_works_to_wikidata_internal(works, progress_callback=None, dryrun=False):
    """
    Internal function to export multiple works to Wikidata with comprehensive logging.

    Args:
        works: QuerySet or list of Work objects
        progress_callback: Optional function(current, total, work) for progress updates
        dryrun: If True, simulate the export without writing to Wikibase
    ...
    stats = {"created": 0, "updated": 0, "skipped": 0, "errors": 0, "total": 0, "log_entries": []}

    works_list = list(works)
    total = len(works_list)

    mode_label = "[DRY-RUN] " if dryrun else ""
    logger.info(f"{mode_label}Starting Wikibase export for {total} work(s) to {settings.WIKIBASE_API_URL}")

    for idx, work in enumerate(works_list, 1):
        stats["total"] += 1

        logger.debug(f"{mode_label}Processing work {idx}/{total}: ID={work.id}, Title='{work.title[:50]}...'")

        # Skip if missing required fields
        if not work.publicationDate:
            logger.info(f"{mode_label}Skipping work {work.id} ('{work.title[:50]}...') - missing work date")
            if not dryrun:
                log_entry = create_export_log(
                    work=work,
                    action="error",
                    error_message="Missing required field: publicationDate",
                    summary="Export skipped due to missing work date",
                )
                stats["log_entries"].append(log_entry)
            stats["errors"] += 1

            if progress_callback:
                progress_callback(idx, total, work)
            continue

        # Attempt export
        logger.debug(f"{mode_label}Calling upsert_publication for work {work.id}")
        action, qid, log_entry = upsert_publication(work, wikibase_client, dryrun=dryrun)
        logger.info(f"{mode_label}Work {work.id} - Action: {action}, QID: {qid}")

        # Update statistics
        if action == "created":
            stats["created"] += 1
            logger.info(f"{mode_label}Created new item {qid} for work {work.id}")
        elif action == "updated":
            stats["updated"] += 1
            logger.info(f"{mode_label}Updated existing item {qid} for work {work.id}")
        elif action == "skipped":
            stats["skipped"] += 1
            logger.debug(f"{mode_label}Skipped work {work.id} (QID: {qid})")
        elif action == "error":
            stats["errors"] += 1
            logger.warning(
                f"{mode_label}Error exporting work {work.id}: {log_entry.error_message[:100] if log_entry and log_entry.error_message else 'Unknown error'}"
            )
    ...
    logger.info(
        f"{mode_label}Wikibase export complete: Created={stats['created']}, Updated={stats['updated']}, Skipped={stats['skipped']}, Errors={stats['errors']}, Total={stats['total']}"
    )

    return stats


def export_works_to_wikidata(works, progress_callback=None):
    """
    Export multiple works to Wikidata/Wikibase (actual write operation).
    ...
    Returns:
        dict with statistics
    """
    return _export_works_to_wikidata_internal(works, progress_callback, dryrun=False)


def export_works_to_wikidata_dryrun(works, progress_callback=None):
    """
    Simulate export of works to Wikidata/Wikibase without writing (dry-run mode).
    ...
    """
    return _export_works_to_wikidata_internal(works, progress_callback, dryrun=True)
