...
@extend_schema(
    ...
    tags=["Gazetteer"],
    parameters=[
        OpenApiParameter("provider", str, OpenApiParameter.PATH, description="Provider id: `nominatim` or `photon`."),
        OpenApiParameter("q", str, OpenApiParameter.QUERY, required=True, description="Search query string."),
        OpenApiParameter(
            "limit", int, OpenApiParameter.QUERY, required=False, description="Maximum number of results (default 5)."
        ),
    ],
    responses={
...
def gazetteer_search(request, provider):
    ...
    if provider not in GEOCODING_SERVICES:
        return JsonResponse(
            {"error": f"Unknown provider: {provider}", "available_providers": list(GEOCODING_SERVICES.keys())},
            status=400,
        )
    ...
    if service_config.get("requires_key", False):
        ...
        if not api_key:
            return JsonResponse({"error": f"Provider {provider} requires an API key"}, status=400)
    ...
    try:
        ...
        response = requests.get(service_config["search_url"], params=params, headers=headers, timeout=10)
        ...
        try:
            ...
            return JsonResponse({"error": "Invalid JSON response from geocoding service"}, status=502)
        ...
        return JsonResponse({"error": f"Geocoding service error: {str(e)}"}, status=502)
...
def gazetteer_reverse(request, provider):
    ...
    try:
        ...
        response = requests.get(service_config["reverse_url"], params=params, headers=headers, timeout=10)
        ...
        return JsonResponse({"error": f"Reverse geocoding service error: {str(e)}"}, status=502)
