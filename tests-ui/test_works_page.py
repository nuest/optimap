...
from works.utils.statistics import STATS_CACHE_KEY, update_statistics_cache
...
class WorksListViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.source = Source.objects.create(name="Test Journal", issn_l="1234-5678")
        ...
        for i in range(75):
            ...
            authors = (
                [f"Author {i}A", f"Author {i}B", f"Author {i}C", f"Author {i}D"] if i % 2 == 0 else [f"Author {i}"]
            )
    ...
    def test_admin_sees_all_works(self):
        ...
        # Create admin user
        admin = User.objects.create_user(username="admin", email="admin@test.com", password="testpass123")
        admin.is_staff = True
        admin.save()
    ...
    def test_page_size_options_in_context(self):
        ...
        self.assertEqual(response.context["page_size_options"], settings.WORKS_PAGE_SIZE_OPTIONS)
