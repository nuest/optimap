...
class ContributionWorkflowE2ETest(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.source = Source.objects.create(name="Test Source", url="https://test.example.com", harvest_enabled=False)
    ...
    def test_complete_contribution_workflow(self):
        ...
        try:
            ...
            if session_cookie:
                driver.add_cookie(
                    {"name": "sessionid", "value": session_cookie.value, "path": "/", "domain": "localhost"}
                )
            ...
            contribute_buttons = driver.find_elements(
                "xpath", "//a[contains(text(), 'Contribute geometry') or contains(text(), 'Add geometry')]"
            )
            ...
            print("  - Geometry added via contribution")
            print("  - Work published by admin")
            print("  - Work visible on landing page and map")
